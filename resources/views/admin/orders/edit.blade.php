...
<div class="max-w-7xl mx-auto">
    <div class="mb-6">
        <p class="text-sm text-earth-brown">Edit pesanan perjalanan pelanggan</p>
    </div>
    ...
    <form method="POST" action="{{ route('admin.orders.update', $order) }}" id="orderForm" class="space-y-6">
        @csrf
        @method('PUT')
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <input 
                        ...
                        id="invoice_date"
                        name="invoice_date" 
                        value="{{ old('invoice_date', $order->invoice_date ? $order->invoice_date->format('Y-m-d') : date('Y-m-d')) }}"
                        class="w-full px-3 py-2.5 border border-cream-dark rounded-lg focus:border-natural-green focus:ring-2 focus:ring-natural-green/20 outline-none transition-all text-sm"
                    >
                </div>
...
// Global variables
let serviceItemsContainer;
let serviceCheckboxes;

// Ensure serviceItemsContainer is available
function getServiceItemsContainer() {
    if (!serviceItemsContainer) {
        serviceItemsContainer = document.getElementById('serviceItems');
    }
    return serviceItemsContainer;
}

// Function to add service item with existing data
function addServiceItemWithData(serviceType, existingData, index) {
    const itemDiv = document.createElement('div');
    itemDiv.className = 'bg-white rounded-xl shadow-sm border border-cream-dark p-6 service-item';
    itemDiv.dataset.serviceType = serviceType;
    itemDiv.dataset.itemIndex = index;
    itemDiv.dataset.itemId = `item-${index}`;
    
    let template = serviceTemplates[serviceType];
    if (!template) return;
    ...
    itemDiv.innerHTML = `
        <div class="flex justify-between items-center mb-4">
            <h4 class="text-md font-semibold text-earth-brown-dark">${serviceLabels[serviceType]}</h4>
            <button type="button" class="text-red-600 hover:text-red-800 text-sm font-medium remove-item" data-service-type="${serviceType}">
                Hapus
            </button>
        </div>
        ${template}
        <div class="mt-4">
            ...
            <textarea name="items[${index}][notes]" rows="2" class="w-full px-3 py-2.5 border border-cream-dark rounded-lg focus:border-natural-green focus:ring-2 focus:ring-natural-green/20 outline-none transition-all text-sm resize-none" placeholder="Catatan khusus untuk layanan ini...">${existingData.notes || ''}</textarea>
        </div>
        <input type="hidden" name="items[${index}][service_type]" value="${serviceType}">
    `;
    
    const container = getServiceItemsContainer();
    if (container) {
        container.appendChild(itemDiv);
    }
    
    // Fill in existing data
    const serviceData = existingData.service_data || {};
    Object.keys(serviceData).forEach(key => {
        const input = itemDiv.querySelector(`input[name="items[${index}][${key}]"], textarea[name="items[${index}][${key}]"]`);
        if (input) {
            if (input.type === 'checkbox') {
                input.checked = serviceData[key] == 1 || serviceData[key] === true;
            } else {
                input.value = serviceData[key] || '';
            }
        }
    });
    
    // Set price
    const priceInput = itemDiv.querySelector(`input[name="items[${index}][price]"]`);
    if (priceInput && existingData.price) {
        priceInput.value = existingData.price;
    }
    
    // Set original_price
    const originalPriceInput = itemDiv.querySelector(`input[name="items[${index}][original_price]"]`);
    if (originalPriceInput && existingData.original_price) {
        originalPriceInput.value = existingData.original_price;
    }
    
    // For hotel, set harga_per_malam if exists
    if (serviceType === 'hotel' && serviceData.harga_per_malam) {
        const hargaPerMalamInput = itemDiv.querySelector(`input[name="items[${index}][harga_per_malam]"]`);
        if (hargaPerMalamInput) {
            hargaPerMalamInput.value = serviceData.harga_per_malam;
        }
        
        // Set up hotel calculation
        const hotelInputs = itemDiv.querySelectorAll('.hotel-date-input, .hotel-room-input, .hotel-price-input');
        ...
        setTimeout(() => {
            const checkInInput = itemDiv.querySelector(`input[name="items[${index}][check_in]"]`);
            const checkOutInput = itemDiv.querySelector(`input[name="items[${index}][check_out]"]`);
            const roomInput = itemDiv.querySelector(`input[name="items[${index}][jumlah_kamar]"]`);
            const pricePerNightInput = itemDiv.querySelector(`input[name="items[${index}][harga_per_malam]"]`);
            
            if (checkInInput && checkOutInput && roomInput && pricePerNightInput) {
                calculateHotelPrice(pricePerNightInput);
            }
        }, 100);
    }
    ...
    // Attach hotel input events
    if (serviceType === 'hotel') {
        const hotelDateInputs = itemDiv.querySelectorAll('.hotel-date-input');
        const hotelRoomInputs = itemDiv.querySelectorAll('.hotel-room-input');
        const hotelPriceInputs = itemDiv.querySelectorAll('.hotel-price-input');
        
        [...hotelDateInputs, ...hotelRoomInputs, ...hotelPriceInputs].forEach(input => {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    @if($order->items->count() > 0)
        itemIndex = {{ $order->items->count() }};
    @endif

    // Use event delegation for remove buttons FIRST - before any items are added
    // This ensures it works for all dynamically added items
    if (serviceItemsContainer) {
        serviceItemsContainer.addEventListener('click', function(e) {
            ...
            if (removeBtn) {
                e.preventDefault();
                e.stopPropagation();
                // Find the parent item container - walk up the DOM tree
                let itemElement = removeBtn.parentElement;
                while (itemElement && itemElement !== serviceItemsContainer) {
                    if (itemElement.hasAttribute('data-item-id') || itemElement.hasAttribute('data-service-type')) {
                        break;
                    }
                    itemElement = itemElement.parentElement;
                }
                
                // Fallback to closest if walking didn't work
                if (!itemElement || itemElement === serviceItemsContainer) {
                    itemElement = removeBtn.closest('[data-item-id]') || removeBtn.closest('[data-service-type]');
                }
                ...
                if (itemElement && itemElement !== serviceItemsContainer) {
                    ...
                    console.error('Could not find parent item element to remove', removeBtn);
                }
            }
        });
    }
    ...
    @if($order->items->count() > 0)
        @php
            $existingItemsData = $order->items->map(function($item) {
                return [
        ...
        // Load existing items after a short delay to ensure DOM is ready
        setTimeout(() => {
            // Ensure container is available
            const container = getServiceItemsContainer();
            if (!container) {
                console.error('Service items container not found');
                return;
            }
            
            existingItems.forEach((item, idx) => {
                const serviceType = item.service_type;
                const currentIndex = idx; // Use array index for existing items
                
                // Add the service item with data
                addServiceItemWithData(serviceType, item, currentIndex);
            });
            
            // Update itemIndex after loading all existing items
